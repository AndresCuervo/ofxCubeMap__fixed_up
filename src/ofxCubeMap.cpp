...
{	
    ...
    // vvvvvvvvvvvv This sux b/c of this: https://github.com/openframeworks/openFrameworks/pull/3313
    //	unsigned char * data_px, * data_nx, * data_py, * data_ny, * data_pz, * data_nz;
    // So just adding the `const ofPixels &` type declaration to the lines below the comment here:
...
//	data_nz = new unsigned char [size * size * 3];
	...
	const ofPixels & data_px = pos_x.getPixels();
    const ofPixels & data_py = pos_y.getPixels();
    const ofPixels & data_pz = pos_z.getPixels();
    
    const ofPixels & data_nx = neg_x.getPixels();
    const ofPixels & data_ny = neg_y.getPixels();
    const ofPixels & data_nz = neg_z.getPixels();
